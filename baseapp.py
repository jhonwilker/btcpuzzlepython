...
def create_public_hash160OLD(priv_key_int):
    priv_key_bytes = priv_key_int.to_bytes(32, byteorder='big')

    # Create a new private key using the ecdsa library
    ...
    compressed_pub_key = b'\x02' + vk.to_string()[:32] if vk.to_string()[0] % 2 == 0 else b'\x03' + vk.to_string()[:32]

    # Generate a Bitcoin address from the public key
    pub_key_hash = hash160(compressed_pub_key)
    pub_key_hash2 = compressed_pub_key

    return pub_key_hash,pub_key_hash2
...
def hash1602(b):
  ...
  ripemd160_hash.update(sha256_hash_digest)
...
# private_keydecimal = 3 #"0x38aaed67231e30956"  # Replace with your actual private key
# wif_result = generate_wif(private_keydecimal)
# print(f"WIF: {wif_result}")

# address = create_public_hash160(private_keydecimal)
# print(f"create_public_hash160/address: {address}")

# wallet = hash160_to_address(address)

# print(f"hash160_to_address/wallet: {wallet}")


# if wallet=="1CUNEBjYrCn2y1SdiUMohaKUi4wpP326Lb" and wif_result=="KwDiBf89QgGbjEhKnhXJuH7LrciVrZi3qYjgd9M7rFU74sHUHy8S":
#     print("OK")
# else:
#      print("CODIGO ERRADO")
