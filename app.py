...
def imprime():
    global lastkey, numchaves, maxkey
    print(f"Chaves buscadas: {numchaves}, Ultima chave: {lastkey}, Chave MAX {maxkey}")

@cuda.jit
def generate_wif(priv_key_int):
  ...
  extended_key = bytes([0x80]) + priv_key_bytes + bytes([0x01])
...
@cuda.jit
def create_public_hash160(priv_key_int):
      # Converter o inteiro da chave privada em bytes
    priv_key_bytes = priv_key_int.to_bytes(32, byteorder='big')

    # Criar uma nova chave privada usando a biblioteca ecdsa
    ...
    # Gerar o hash160 a partir da chave pública comprimida
...
def hash160_to_address(hash160: bytes) -> str:
    ...
    checksum_bytes = checksum(versioned_payload)
    full_payload = versioned_payload + checksum_bytes
    return base58.b58encode(full_payload).decode('utf-8')

@cuda.jit
def gpu_operation(wallets, results, found_flag): 
    idx = cuda.grid(1)
    if idx < results.size:
        lastkeydecimal = int(results[idx], 16)
        
        #address = create_public_hash160(lastkeydecimal)
        priv_key_bytes = lastkeydecimal.to_bytes(32, byteorder='big')
        priv_key = ecdsa.SigningKey.from_string(priv_key_bytes, curve=ecdsa.SECP256k1)
        compressed_pub_key = priv_key.verifying_key.to_string("compressed")
        pub_key_hash = hashlib.new('ripemd160', hashlib.sha256(compressed_pub_key).digest()).digest()
        address =  pub_key_hash
        
        #wallet = hash160_to_address(address)
        versioned_payload = b'\x00' + address 
        #checksum_bytes = checksum(versioned_payload)
        hash1 = hashlib.sha256(versioned_payload).digest()
        hash2 = hashlib.sha256(hash1).digest()
        checksum_bytes = hash2[:4]               
        full_payload = versioned_payload + checksum_bytes
        wallet = base58.b58encode(full_payload).decode('utf-8') 
        wif_result = generate_wif(lastkeydecimal)
        
        for i in wallets:
            if i == wallet:
                found_flag[idx] = 1
                results[idx] = wif_result
                return
...
if __name__ == "__main__":
    ...
    try:
        ...
        n = 1024  # Ajuste conforme necessário
        threads_per_block = 128
        blocks_per_grid = (n + (threads_per_block - 1)) // threads_per_block

        while int(lastkey, 16) < int(maxkey, 16):

            lastkey_decimal = int(lastkey, 16)
            keys_to_check = [hex(lastkey_decimal + i) for i in range(n)]
            lastkey = hex(lastkey_decimal + n)

            results = np.array(keys_to_check, dtype=np.object_)
            results_gpu = cuda.to_device(results)
            found_flag = np.zeros(n, dtype=np.int32)
            found_flag_gpu = cuda.to_device(found_flag)

            gpu_operation[blocks_per_grid, threads_per_block](wallets, results_gpu, found_flag_gpu)
            results = results_gpu.copy_to_host()
            found_flag = found_flag_gpu.copy_to_host()

            # for idx, result in enumerate(results):
            #     if found_flag[idx] == 1:
            #         print(f"ACHEI: WIF: {result}")
            #         with open("chaves_encontradas.txt", 'a') as file:
            #             file.write(f"{result}\n")
            #     else:
            #         numchaves += 1
            #     imprime()
