...
def gpu_worker(active_workers, lock, file_path, stop_event):
    ...
    # Incrementa a contagem de workers ativos
    ...
    def workerrun(): 
        ...
        lastkeydecimal = int(lastkey, 16)
        address    = baseapp.create_public_hash160(lastkeydecimal)
        wallet     = baseapp.hash160_to_address(address)
        wif_result = baseapp.generate_wif(lastkeydecimal)       

        for i in wallets:
            # Verifica se o chave gerado é tem na wallet
            ...
            if i==wallet:
                ...
                stop_event.set()  # Sinaliza para parar todos os processos
            else:
                lastkey = increment_hex(lastkey)                
        ...
        with open("lastkey.txt", 'a') as file:
            file.write(f"{lastkey}\n")
    ...
    try:
        ...
        # Decrementa a contagem de workers ativos quando o processo termina
        with active_workers.get_lock():
            active_workers.value -= 1

def monitor_active_workers(active_workers, stop_event):
    try:
        while not stop_event.is_set():
            with active_workers.get_lock():
...
if __name__ == "__main__":    
    ...
    processes = []
    active_workers = multiprocessing.Value('i', 0)  # Contador de workers ativos
    lock = multiprocessing.Lock()  # Lock para sincronizar a escrita no arquivo
    file_path = "chaves_encontradas.txt"  # Caminho para o arquivo de saída
    stop_event = multiprocessing.Event()  # Evento para sinalizar a parada
    ...
    # Mantém o processo principal rodando para capturar o sinal de interrupção
    # try:
    #     while not stop_event.is_set():
    #         #time.sleep(1)
    # except KeyboardInterrupt:
